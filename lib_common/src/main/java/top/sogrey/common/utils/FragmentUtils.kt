    ...
    companion object {
        ...
        private fun getArgs(fragment: Fragment): Args? {
            ...
            return if (bundle == null || bundle.getInt(ARGS_ID) == 0) null else Args(
                bundle.getInt(
                    ARGS_ID
                ), bundle.getBoolean(ARGS_IS_HIDE), bundle.getBoolean(ARGS_IS_ADD_STACK)
            )
        }
        ...
        private fun operateFragment(
            ...
            if (srcFragment != null && srcFragment.isRemoving) {
                logE(srcFragment.javaClass.name + " is isRemoving")
                return null
            }
        ...
        fun findFragment(
            ...
            val fragments = getFragments(fragmentManager)
            return if (fragments.isEmpty()) null else fragmentManager.findFragmentByTag(
                fragmentClass.name
            )
        }
